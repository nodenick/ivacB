(function completeProcessFlow() {
    ...
    // Function to create an AJAX request with proxy
    function createProxiedRequest(options) {
      const proxy = getNextProxy();
      
      // Set up beforeSend to add proxy authentication
      const originalBeforeSend = options.beforeSend;
      options.beforeSend = function(xhr) {
        xhr.setRequestHeader('Proxy-Authorization', 'Basic ' + btoa(proxy.username + ':' + proxy.password));
        
        // Add custom headers to indicate proxy usage
        xhr.setRequestHeader('X-Proxy-Server', proxy.server);
        xhr.setRequestHeader('X-Proxy-User', proxy.username);
      ...
      // Log proxy usage
      console.log(`Using proxy: ${proxy.server} with user: ${proxy.username}`);
    ...
    // The following functions are updated to use the proxied request method
    function sendOtp() {
